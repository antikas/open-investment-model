...
| `target_id` | varchar (FK → {PM-04, PM-01}) | The thing held — a PM-04 Portfolio Company when `holding_type = portfolio_company`, a PM-01 Fund when `holding_type = fund`. |
...
- The position is held through one or more Legal Vehicles (PM-05); cash flows record at vehicle grain and roll up here. The **multi-vehicle** case — one investment held through several vehicles — is already carried by `PM-05.investment_id`: it is a many-to-one foreign key, so every vehicle whose `investment_id` resolves to this investment is one of the vehicles it is held through. The relationship is that existing edge read in the one-investment-to-many-vehicles direction; no back-pointer on PM-09 is added (a single foreign key could not hold the set).

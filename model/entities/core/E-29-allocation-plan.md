...
- **Two directions, version-grained here, in-force there.** `subject_id` is carried on *every* version of a plan and points at the portfolio it governs, so traversing from a portfolio yields the plan's full version history. The reverse convenience pointer — a portfolio's reference to its *single in-force* plan version — is `E-03.governing_plan_id`; it exists so the plan currently in force is reached without scanning every version's effective window. The two are distinct edges, not one relationship stated twice.

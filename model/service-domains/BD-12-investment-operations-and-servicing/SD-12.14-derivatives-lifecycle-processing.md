...
- **Relationship to CDM:** this is the BD-12 Service Domain where the CDM boundary is sharpest. **The Common Domain Model, now stewarded by FINOS and developed from ISDA's derivatives CDM lineage, models lifecycle events in a machine-executable form at transaction grain.** SD-12.14 is the buy-side processing capability that runs those events against the portfolio-level position. Where an implementation runs CDM, DR-02's `cdm_trade_ref` links to the authoritative lifecycle representation. Contract and lifecycle-event details belong in CDM. OpenIM covers the position, exposure and investment-firm relationship around them.

## Standards

- **FINOS Common Domain Model (CDM)**, developed from ISDA's derivatives CDM lineage, is the machine-executable representation of the derivative product, trade and lifecycle events processed here.

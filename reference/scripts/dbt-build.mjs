#!/usr/bin/env node
/**
 * dbt-build.mjs — run the canonical-data dbt pipeline.
 *
 ...
 *
 * Platform split (same as the cross-language smoke): on Windows the
 ...
 * locally. Either way:
 *
 *   1. the duckdb DATABASE FILE is created on ext4 — NOT on the 9p
 ...
 *      dbt-build.sh computes from this checkout's repo root
 ...
 *   - the duckdb ext4 path if the caller overrode it via env;
...
function run() {
  ...
  if (isWin) {
    ...
    // explicit UV_PROJECT_ENVIRONMENT override is forwarded too; when
  ...
  log(`duckdb file lands on WSL2-native ext4 — checkout-keyed default under $HOME/.local/share/agentinvest/duckdb/`);

...
The catch this serde is built around (the bd09 precedent, OIM-114): the Restate SDK **re-wraps any
...
The malformed-body invariant (OIM-187 — why this is its own SSOT module): a bare ``json.loads(buf)``
...
not a ``ValueError`` — which escaped to a status-less 500 (OIM-187 cycle-2). Result: a malformed
...
class PassThroughJsonSerde(restate.serde.Serde[Any]):
    ...
    def deserialize(self, buf: bytes) -> Any:
        ...
        try:
            ...
            # which escaped → a status-less 500 (OIM-187 cycle-2). The serde does NOTHING but parse,
